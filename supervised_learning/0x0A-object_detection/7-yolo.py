...
class Yolo:
    ...
    def preprocess_images(self, images):
        """preprocesses the imagaes - resize then rescale"""
        iw = self.model.input.shape[1]
        ih = self.model.input.shape[2]
        ni = len(images)
        ...
        for i, pic in enumerate(images):
            ...
            pic = cv2.resize(pic, (iw, ih), interpolation=cv2.INTER_CUBIC) 
            n[i] = pic / 255
    ...
    def show_boxes(self, image, boxes, box_classes, box_scores, file_name):
        """
            show_boxes - shows the results - shows our image with the detection
        ...
        for i, b in enumerate(boxes):
            ...
            bc = box_classes[i]
            bs = box_scores[i]
            result = "{} {:.2f}".format(self.class_names[bc], bs)
            ready = cv2.putText(
                img, result, (X1, Y1 - 5),
    ...
    def predict(self, folder_path):
        """
            dd the public method def predict(self, folder_path):
        ...
        """
        oimg, file_paths = self.load_images(folder_path)
        pimg, o_shape = self.preprocess_images(oimg)

        predictions = []
        anchors = []

        outputs = self.model.predict(pimg, verbose=1)

        for i in range(len(pimg)):
            out = [x[i] for x in outputs]
            b, bc, bcp = self.process_outputs(out, o_shape[i])
